...
While signed in as any of these, **Settings → Developer · demo accounts** lets you switch straight to the other side of a deal (business ↔ funder) without retyping credentials. It renders only for these demo accounts and can only sign into another one of them ([src/lib/demo.ts](src/lib/demo.ts)).
